...
- 普通用户注册成功后默认处于 `disabled`，必须由管理员启用后才能登录；启用后的登录、单设备会话和用户数据隔离逻辑保持既有规则。
...
- Modify: `frontend/package.json`、`frontend/package-lock.json`（补齐现有 `main.js` 已使用的 `tdesign-vue-next@1.19.1` 直接依赖，并与锁文件同步）
...
若构建前发现 `main.js` 的既有 `tdesign-vue-next/es/style/index.css` 无法解析，使用锁文件已存在的 `tdesign-vue-next@1.19.1` 补齐 `frontend/package.json` 直接开发依赖，并同步 `frontend/package-lock.json`，不升级其他包。
...
- 管理员 Token 登录/鉴权路径会清理过期 Token，并限制内存中最多保留 32 个活动 Token。
- 普通登录创建会话前在同一事务内重新确认用户为 `active`，并使用 `FOR UPDATE` 行锁语义协调管理员禁用。
- 新增 Token 生命周期和登录/禁用竞态回归测试；列表测试覆盖手机号过滤和非 `user` 角色排除。

## 完成前核对

- [x] 对照设计逐项确认登录、列表、搜索、启用/禁用和权限边界。
- [x] `go test ./backend/flutter_api -count=1` 通过。
- [x] `npm run build` 通过（当前环境使用等价的 `pnpm run build` 执行）。
- [x] `git diff --check` 通过。
- [x] 本次提交未包含 `data/stock.db`、`backend/data/stock.db`、T0 缓存或其他无关文件。
